...
func TestProfileEditFormText(t *testing.T) {
	server := newFullMockServer(t)
	defer server.Close()
	factory := newTestClientFactory(server)
	...
	root := newTestRootCmd()
	root.AddCommand(buildTestProfileCmd(factory))
	out := runCmd(t, root, "profile", "edit-form")

	mustContain(t, out, "Username:")
	mustContain(t, out, "testuser")
	mustContain(t, out, "Email:")
	mustContain(t, out, "testuser@example.com")
	mustContain(t, out, "Bio:")
}

func TestProfileEditFormJSON(t *testing.T) {
	server := newFullMockServer(t)
	defer server.Close()
	factory := newTestClientFactory(server)
	...
	root := newTestRootCmd()
	root.AddCommand(buildTestProfileCmd(factory))
	out := runCmd(t, root, "profile", "edit-form", "--json")

	var fd ProfileFormData
	if err := json.Unmarshal([]byte(out), &fd); err != nil {
		t.Fatalf("invalid JSON: %v\noutput: %s", err, out)
	}
	if fd.Username != "testuser" {
		t.Errorf("expected username=testuser, got %s", fd.Username)
	}
	if fd.Email != "testuser@example.com" {
		t.Errorf("expected email=testuser@example.com, got %s", fd.Email)
	}
	if !fd.IsEmailConfirmed {
		t.Error("expected is_email_confirmed=true")
	}
}
