...
// Extracted from X's web client JS bundles. These must be kept in sync when X
// updates their frontend — missing keys cause empty responses.
...
// DefaultFieldToggles contains the field toggle flags sent alongside features.
...
func (c *Client) GraphQL(ctx context.Context, queryHash, operationName string, variables map[string]any, features map[string]bool) (json.RawMessage, error) {
	...
	togglesJSON, err := json.Marshal(DefaultFieldToggles)
	if err != nil {
		return nil, fmt.Errorf("marshal graphql field toggles: %w", err)
	}
