...
func (p *Provider) RegisterCommands(parent *cobra.Command) {
	...
	liCmd.AddCommand(newMessagesCmd(p.ClientFactory))
	liCmd.AddCommand(newCompaniesCmd(p.ClientFactory))
	liCmd.AddCommand(newJobsCmd(p.ClientFactory))
	...
	liCmd.AddCommand(newGroupsCmd(p.ClientFactory))
	liCmd.AddCommand(newEventsCmd(p.ClientFactory))
	liCmd.AddCommand(newSkillsCmd(p.ClientFactory))
