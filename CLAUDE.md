...
## Overview
Go CLI binary (`integrations`) that AI agents call inside Docker containers to interact with external services. Supports Gmail and Google Calendar.
...
- `internal/auth/` — Google OAuth token management with auto-refresh (Gmail + Calendar)
...
## Testing
- Both providers use `ServiceFactory` for dependency injection
- Tests use `httptest.NewServer` to mock APIs via `newFullMockServer(t)`
- Coverage target: 80%+ (currently 93.2% gmail, 92.9% calendar)
